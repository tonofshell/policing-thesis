...
merged_crime_data %>% filter(!is.na(date)) %>% 
  ...
  labs(title = "Frequencies of Crime Reports by Time of Day", subtitle = "Within UCPD's patrol area, from January 1, 2010 to December 2, 2019", y = "Day of Week", x = "Hour of Day", fill = "Log Number of \nReported Crimes")
```
...
merged_crime_data %>% filter(in_ucpd_bound, !is.na(responding_dept)) %>% mutate(responding_dept = factor(responding_dept, c("cpd", "ucpd", "both"), c("CPD", "UCPD", "Both"))) %>% count(primary_type, responding_dept) %>% na.omit() %>% crosstab_percent(vars = c("responding_dept")) %>% ggplot(aes(x = responding_dept, color = primary_type, fill =  primary_type, y = percent)) + geom_col() + coord_flip() + scale_y_continuous(labels = percent) + scale_fill_manual(values = color_pal(7, "discrete", levels = length(unique(merged_crime_data$primary_type)))) + scale_color_manual(values = color_pal(7, "discrete", levels = length(unique(merged_crime_data$primary_type)))) + labs(title = "Categories of Crimes by Responding Department", subtitle = "Within UCPD's patrol area, from January 1, 2010 to December 2, 2019", x = "Responding Department", y = "Percent", color = "Crime\nCategory", fill = "Crime\nCategory") + theme_day(base_family = "Pragati Narrow", base_size = 18)
```
...
hyde_park_map + stat_density2d(data = filter(merged_crime_data, !is.na(responding_dept)) %>% mutate(responding_dept = factor(responding_dept, c("cpd", "ucpd", "both"), c("CPD", "UCPD", "Both"))), aes(x = lon_x, y = lat_y, fill = stat(level)),  geom="polygon", alpha = 0.75) +  scale_fill_gradientn(colors = color_pal(5, "heatmap")) + facet_wrap(~ responding_dept) + labs(title = "Location of Crimes by Responding Department", subtitle = "For all crimes, from January 1, 2010 to December 2, 2019", fill = "Crime Density") + theme_map(base_family = "Pragati Narrow", base_size = 18)
```
...
hyde_park_map + stat_density2d(data = filter(merged_crime_data, in_ucpd_bound, primary_type %in% top_categories$primary_type), aes(x = lon_x, y = lat_y, fill = stat(level)),  geom="polygon", alpha = 0.75) +  scale_fill_gradientn(colors = color_pal(5, "heatmap"), breaks = c(2000, 4000, 6000)) + facet_wrap(~ primary_type) + labs(title = "Top 11 Categories of All Crimes by Location", subtitle = "Within UCPD's patrol area, from January 1, 2010 to December 2, 2019", fill = "Crime Density") + theme_map(base_family = "Pragati Narrow", base_size = 18)
```
